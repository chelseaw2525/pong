...
public abstract class GDV5 extends Canvas implements Runnable, KeyListener {
	...
	private static int MAX_WINDOW_X = 700;
	...
	public static int collisionDirection(Rectangle paddle, Rectangle projectile, int dx1, int dy1, int dy2) {

        // calculate previous location
        int previousXPos1 = (int) projectile.getX() - dx1;
        int previousYPos1 = (int) projectile.getY() - dy1;
        int height1 = (int) projectile.getHeight();
        int width1 = (int) projectile.getWidth();
        int previousYPos2 = (int) paddle.getY() - dy2;
        int height2 = (int) paddle.getHeight();
        int width2 = (int) paddle.getWidth();
        int result = 0; // default intersects from right

        if (previousYPos1 + height1 <= previousYPos2 + (height2 / 5) && projectile.getMaxY() >= paddle.getY()) {
            // intersects from top 1/5 of paddle
            result = 1;
        } else if (previousXPos1 + width1 <= paddle.getX() + width2 && projectile.getMaxX() >= paddle.getX()) {
            // intersects from left
            result = 2;
        } else if (previousYPos1 >= previousYPos2 + (height2 * 4 / 5) && projectile.getMaxY() <= paddle.getMaxY()) {
            // intersects from bottom 1/5 of paddle
            result = 3;
        }
        return result;
    }
